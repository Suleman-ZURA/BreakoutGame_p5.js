...
class Ball {
  ...
  update() {
    ...
    // BALL COLLISION WITH SLAB
    if (this.y + this.r >= slab.y &&this.x + this.r >= slab.x 
      && this.x -this.r <= slab.x + slab.w && this.y + this.r <= slab.y + slab.h) {
      this.moveBallY = -this.ballSpeed;
      hitSlabSound.play();
    }
    this.x += this.moveBallX;
    this.y += this.moveBallY;
  }
  ...
  for (let i = 0; i < rows && !brickHit; i++) {
    for (let j = 0; j < columns && !brickHit; j++) {
      ...
      if (brick) {
        ...
        // Ball and Brick Boundaries
        let ballLeft = this.x - 30;
        let ballRight = this.x + 30;
        let ballTop = this.y - 30;
        let ballBottom = this.y + 30;
